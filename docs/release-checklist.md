...
- An 85% minimum mutation score was approved. The current 1.1.2 candidate measures 87.85% with 100% mutation-code coverage, four timeouts, and no ignored source.
...
1. Create `release/v1.1.2`, update its dated changelog section and `docs/releases/1.1.2.md`, and open a pull request to `main`.
...
1. Merge the reviewed `release/v1.1.2` pull request into `main`; this automatically starts the release workflow and derives version `1.1.2`.
...
3. Confirm the immutable `1.1.2` tag and GitHub Release target the reviewed commit and contain the expected artifacts and provenance.
4. Verify GitHub and Packagist identify `AGPL-3.0-or-later` for 1.1.2 while 1.0.x tags retain their original MIT terms.
5. Install the exact public package into clean plain PHP, Laravel, and Symfony fixtures:

```bash
composer require fleetbase/fleetbase-php:1.1.2
composer install --no-dev --optimize-autoloader
```

6. Confirm the public API reference renders the checked-in 1.1.2 PHP examples.
